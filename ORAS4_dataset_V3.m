...
cd C:\Users\samst\Documents\BATS_VPN\ORAS4\data\
dir_struct1=dir('C:\Users\samst\Documents\BATS_VPN\ORAS4\data\*so*');
dir_struct2=dir('C:\Users\samst\Documents\BATS_VPN\ORAS4\data\*theta*');

lat=ncread('so_oras4_1m_1958_grid_1x1.nc','lat');
lon=ncread('so_oras4_1m_1958_grid_1x1.nc','lon');
xcut=-79.5:-40.5;ycut=20.5:42.5;
% [xgrid,ygrid,dgrid]=meshgrid(xcut,ycut,depth);
% xgrid=rot90(xgrid);ygrid=rot90(ygrid);dgrid=double(rot90(dgrid));
depth=ncread('so_oras4_1m_1958_grid_1x1.nc','depth');
...
% m_proj('equidistant','lon',[-80 -30],'lat',[25 50]);
% % m_proj('equidistant','lon',[-180 180],'lat',[-90 90]);
% m_coast('patch',[0.7 0.7 0.7]);
% hold on
% % m_contourf(x,y,squeeze(salt(:,:,1,1)));
% % m_scatter(x_cut(:),y_cut(:));
% m_contourf(x_cut,y_cut,squeeze(salt(lon_idx,lat_idx,1,1)));
% % m_contourf(x_cut,y_cut,squeeze(temp(lon_idx,lat_idx,1,1)));

for i=1:length(dir_struct1)+length(dir_struct2)
    
    if i<=length(dir_struct1)
        ds=dir_struct1(i);
        disp(ds.name)
        salt=ncread(ds.name,'so');
        salt_cut=salt(lon_idx,lat_idx,:,:);
        tmp_t=datestr(double(ncread(ds.name,'time'))+datenum(1957,09,01),23);
        
        for tt=1:length(tmp_t)
            tmp_tstr=tmp_t(tt,:);
            tmp_tstr2(1,1)=str2num([tmp_tstr(7:10) tmp_tstr(1:2) tmp_tstr(4:5)]);
            tmp_tstr2(1,2)=0000;
            tmp_t_dec=cnv_date(tmp_tstr2);
%             salt_month=[salt_month interp1(depth,squeeze(mean(mean(salt_cut(:,:,:,tt),1,...
%                 'omitnan'),2,'omitnan')),[0:2:600]')];
            
             BATS_salt_month=[BATS_salt_month interp1(depth,...
                 squeeze(salt_cut(b_lon_idx,b_lat_idx,:,tt)),[0:2:600]')];
            
            for ii=1:length(lon_idx)
                for iii=1:length(lat_idx)
                    count2=count2+1;
                    salt_grid(:,count2)=interp1(depth(depth_idx),...
                        squeeze(salt_cut(ii,iii,depth_idx,tt)),[0:2:600]');
        ...
        count=count+1;
        ds=dir_struct2(count);
        disp(ds.name)
        temp=ncread(ds.name,'thetao');
        ...
        for tt=1:length(tmp_t)
            ...
            tmp_t_dec=cnv_date(tmp_tstr2);
             ...
             BATS_temp_month=[BATS_temp_month interp1(depth,...
                 squeeze(temp_cut(b_lon_idx,b_lat_idx,:,tt)),[0:2:600]')];
                        
            for ii=1:length(lon_idx)
                for iii=1:length(lat_idx)
                    count3=count3+1;
                    temp_grid(:,count3)=interp1(depth(depth_idx),...
                        squeeze(temp_cut(ii,iii,depth_idx,tt)),[0:2:600]');
                    lon_vec(count3)=xcut(ii);
                    lat_vec(count3)=ycut(iii);
                    time(count3)=tmp_t_dec;
                end
            end
        end
     end
...
thick_grid=NaN([size(lon_grid) length(1958:.25:2018)]);
ulim_grid=NaN([size(lon_grid) length(1958:.25:2018)]);
llim_grid=NaN([size(lon_grid) length(1958:.25:2018)]);
PV_grid=NaN([size(lon_grid) length(1958:.25:2018)]);
intensity_grid=NaN([size(lon_grid) length(1958:.25:2018)]);
stemp_grid=NaN([size(lon_grid) length(1958:.25:2018)]);
hc_grid=NaN([size(lon_grid) length(1958:.25:2018)]);
wtemp_grid=NaN([size(lon_grid) length(1958:2018)]);
outcrop_grid=zeros([size(lon_grid) length(1958:2018)]);
...
for i=1:length(lon_list)
    for ii=1:length(lat_list)
        ...
        for iii=1958:.25:2018
            i
            ii
            count
            
            count=count+1;
            time_idx=tmp_time>iii & tmp_time<iii+0.25;
            mn_temp=mean(tmp_temp(:,time_idx),2,'omitnan');
            ...
            if sum(s_msk)>2
                intensity_grid(ii,i,count)=...
                    trapz(pres(idx:idy),(inpaint_nans(1e-10-q(idx:idy))));
            end
