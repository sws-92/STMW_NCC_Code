...
% Load in casts created from the historical bottle data
load HS_casts
CTD_data=HS_casts;
...
d_grad_cont=zeros(600/5,length(castlist));
...
CTD_data(isnan(CTD_data(:,6)),6)=...
    gsw_depth_from_z(gsw_z_from_p(CTD_data(isnan(CTD_data(:,6)),5),...
    CTD_data(isnan(CTD_data(:,6)),3)));
...
for i=1:length(castlist)
    ...
    if max(rel_cast(:,5))>490 && min(rel_cast(:,5))<100
        ...
        [~,q,p_ave]=sw_bfrq(rel_cast(:,9),rel_cast(:,7),rel_cast(:,5),rel_cast(1,3));        q=interp1(p_ave,q,rel_cast(:,5));
        q=movmean(q,35);
        ...
        %         ylim([0 600]);
        %         min(q(idx:idy))
        %         legend('smth prof.','Stevens thickness','19-17C isotherms',...
        %             'location','best');
        %         xlabel('PV/ m^{-1} s^{-1}');
        %         pause
        %         close all
        ...
        try
            temp_cont(:,i)=interp1(rel_cast(:,6),inpaint_nans(CT),5:5:600);
        catch
            temp_cont(:,i)=NaN;
        end
        ...
        % Calulate core pv between 17.5-18.5oC isotherms
        if sum(s_msk)>2
            try
        ...
        % Create pv contour
        try
            pv_cont(:,i)=interp1(rel_cast(:,5),inpaint_nans(q),5:5:600);
        catch
            pv_cont(:,i)=NaN;
        end
...
% Concatonate 1955-1988 data with 1988-2018 data
BATS=load('20191010_MWdataset.mat');
ed_final=[ed_final;BATS.ed_final];
ed_limits_final=[ed_limits_final;BATS.ed_limits_final];
stevens_limits_final=[stevens_limits_final;BATS.stevens_limits_final];
low_pv_final=[low_pv_final;BATS.low_pv_final];
pv_data_final=[pv_data_final;BATS.pv_data_final];
time_final=[time_final;BATS.time_final];
mld_final=[mld_final;BATS.mld_final];
temp_cont_final=[temp_cont_final BATS.temp_cont_final];
pv_cont_final=[pv_cont_final BATS.pv_cont_final];
dens_cont_final=[dens_cont_final BATS.dens_cont_final];
d_grad_cont_final=[d_grad_cont_final BATS.d_grad_cont_final];
tgrad_cont_final=[tgrad_cont_final BATS.tgrad_cont_final];
worthington_intensity_final=[worthington_intensity_final;BATS.worthington_intensity_final];
heat_content_final=[heat_content_final;BATS.heat_content_final];
stmw_heat_content_final=[stmw_heat_content_final;BATS.stmw_heat_content_final];
stevens_thickness_final=[stevens_thickness_final;BATS.stevens_thickness_final];
worthington_final=[worthington_final;BATS.worthington_final];
s_spice_final=[s_spice_final;BATS.s_spice_final];
gmax_final=[gmax_final;BATS.gmax_final];
UO_heat_content_final=[UO_heat_content_final;BATS.UO_heat_content_final];
thickness_def_final=[thickness_def_final;BATS.thickness_def_final];
pv_def_final=[pv_def_final;BATS.pv_def_final];
t_def_final=[t_def_final;BATS.t_def_final];
% aou_final=[aou_final;BATS.aou_final];
